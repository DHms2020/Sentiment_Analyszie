...
PAD_IDX = 25001
# 配置tensorboard输出路径
Tensorboard_LSTM_path = "Transformer_tensboard"
...
if __name__ == "__main__":
    ...
    model = Model(vocab_len=INPUT_DIM, padding_id=pad_id, pad_size=200, dropout_rate=DROPOUT,num_class=OUTPUT_DIM,device=device)
    pretrain_emb = np.load('my_w2vec_embedding.npy')
    ...
    train_loader = DataLoader(train_data, batch_size=64, shuffle=True, collate_fn=PadCollateAllBatchSame(dim=0))
    test_loader = DataLoader(test_data, batch_size=64, shuffle=False, collate_fn=PadCollateAllBatchSame(dim=0))
    # # 定义tensorboard
    ...
    torch.save(model.state_dict(), './models/Transformer_para.pth')
